  ...
  Future<void> _runTests() async {
    try {
      ...
      try {
        ...
        print("   Trying to get document Bm2MVWjjZiASW0spGMZy by ID...");
        final specificDoc = await FirebaseFirestore.instance
            .collection('users')
            .doc('Bm2MVWjjZiASW0spGMZy')
            .get();
        ...
        if (specificDoc.exists) {
          ...
          print("   ❌ Document Bm2MVWjjZiASW0spGMZy NOT found!");
        }
        ...
        print("SUCCESS: Firestore query completed! Found ${testDoc.docs.length} documents");
        if (testDoc.docs.isNotEmpty) {
          for (var doc in testDoc.docs) {
            print("   - Document ID: ${doc.id}, data: ${doc.data()}");
          }
        } else {
          print("   No documents found in query (but we see one in Firebase Console!)");
        }
      } catch (e) {
